...
export function KOSchedule({ tranche, quotes }: Props) {
  ...
  return (
    <section className="card mt-4 p-4">
      ...
      {/* ── Mobile card layout (visible <md) — each observation is a
          self-contained card. Fits in one phone screen, no horizontal
          scroll required. ──────────────────────────────────────────── */}
      <div className="md:hidden divide-y divide-[var(--line)] rounded-xl border border-[var(--line)] bg-[var(--surface)] overflow-hidden">
        {sched.map((o) => {
          ...
          return (
            <div key={o.n} className={clsx("px-3 py-3", isNext && "bg-accent-50 dark:bg-accent-900/30")}>
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="flex-shrink-0 rounded-full bg-[var(--surface-2)] border border-[var(--line)] px-2 py-0.5 text-[11px] font-semibold tabular">
                    #{o.n}
                  </span>
                  <span className="tabular text-[12px] font-medium truncate">{o.date}</span>
                  <span className="flex-shrink-0 text-[10.5px] text-[var(--text-muted)]">
                    KO {(o.koPct * 100).toFixed(0)}%
                  </span>
                </div>
                {/* Worst-of / KO badge (compact, right-aligned) */}
                <div className="flex-shrink-0">
                  {past && memo.memorisedKOAtObs === o.n ? (
                    <span className="badge safe text-[10px]" title="Memorised KO">🔒 KO</span>
                  ) : past ? (
                    allResolvedPast && worst && worst.delta >= 0 ? (
                      <span className="badge safe text-[10px]">KO</span>
                    ) : allResolvedPast && worst ? (
                      <span className="badge moderate text-[10px]">Survived</span>
                    ) : worst ? (
                      <span className="badge moderate text-[10px]">{worst.delta >= 0 ? "Likely KO" : "Likely OK"}</span>
                    ) : <span className="badge moderate text-[10px]">—</span>
                  ) : worst ? (
                    <span className={clsx(
                      "badge text-[10px]",
                      worst.delta >= 0 ? "safe"
                        : worst.delta >= -5 ? "near-ki"
                        : worst.delta >= -15 ? "moderate"
                        : "high-risk"
                    )}>
                      {worst.delta >= 0 ? "+" : ""}{worst.delta.toFixed(1)}%
                    </span>
                  ) : (
                    <span className="badge moderate text-[10px]">—</span>
                  )}
              ...
              {showInitialFx && (
                <div className="space-y-1">
                  {perSym.map(({ u, koPx, delta, histClose }) => {
                    ...
                    return (
                      <div key={u.symbol} className="grid grid-cols-[auto_1fr_auto] items-baseline gap-x-2 text-[11px]">
                        <span className="font-mono font-semibold w-12 truncate">{u.symbol}</span>
                        <div className="tabular text-[var(--text-muted)] flex flex-wrap gap-x-2">
                          <span>KO {formatPx(koPx)}</span>
                          {past && histClose != null && <span>close {formatPx(histClose)}</span>}
                          {memorisedAtThisRow && <span className="text-warning font-semibold">🔒 touched</span>}
                          {memorisedEarlier && <span title={`Memorised at obs #${firstTouched}`}>✓ memo</span>}
                        </div>
                        {delta != null && (
                          <span className={clsx(
                            "tabular text-[11px] font-semibold tabular-nums",
